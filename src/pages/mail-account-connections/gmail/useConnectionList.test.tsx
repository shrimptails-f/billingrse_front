...
const fetchConnectionListMock = vi.fn();

vi.mock('./gmail-oauth.api', () => ({
  fetchConnectionList: () => fetchConnectionListMock(),
}));
...
describe('useConnectionList', () => {
  beforeEach(() => {
    fetchConnectionListMock.mockReset();
  });
  ...
  it('returns connection list on success', async () => {
    ...
    fetchConnectionListMock.mockResolvedValueOnce(mockData);
  ...
  it('returns empty items array when no connections exist', async () => {
    fetchConnectionListMock.mockResolvedValueOnce({ items: [] });
    const wrapper = createWrapper();
    const { result } = renderHook(() => useConnectionList(), { wrapper });
  ...
  it('exposes error state when fetch fails', async () => {
    fetchConnectionListMock.mockRejectedValueOnce(new Error('fetch failed'));
    const wrapper = createWrapper();
