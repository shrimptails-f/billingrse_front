...
import { useCompleteGmailOAuth } from '../hooks/useCompleteGmailOAuth';
import type { GmailOAuthErrorBody } from '../types/gmail-oauth.types';

type Status = 'pending' | 'success' | 'error' | 'missing';

const callbackPath = '/mail-account-connections/gmail/callback';
const connectionPath = '/mail-account-connections/gmail';
...
const getGmailOAuthErrorCode = (error: unknown): GmailOAuthErrorBody['code'] | undefined => {
  if (!(error instanceof ApiError)) {
    return undefined;
  }

  const directCode = getApiErrorCode(error);
  if (directCode) {
    return directCode;
  }
...
const mapCallbackError = (error: unknown): string => {
  if (error instanceof ApiError) {
    const code = getGmailOAuthErrorCode(error);
